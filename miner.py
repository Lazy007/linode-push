...
address = str(inpAdd)
print(Fore.YELLOW,'\nBitcoin Wallet Address ===>> ',str(address))
print(Fore.MAGENTA,'\n------------------------------------------------------------------------------')
...
def BitcoinMiner(restart=False):
    ...
    print('Working on current Network height',Fore.WHITE, work_on)
...
if __name__ == '__main__':
    Process(target=newBlockListener).start()
    time.sleep(2)
    Process(target=BitcoinMiner).start()